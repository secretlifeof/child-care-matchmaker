...
class PreferenceStrength(str, Enum):
    ...
    EXCLUDE = "EXCLUDE"             # Must NOT have (dealbreaker if present)
...
class TimeSlot(BaseModel):
    ...
    start_minute: int | None = Field(None, ge=0, le=1439)  # 0-1439 (24*60-1)
    end_minute: int | None = Field(None, ge=0, le=1440)    # 0-1440 (24*60)
    start: str | None = Field(None, pattern=r"^([01]?[0-9]|2[0-3]):[0-5][0-9]$")  # HH:MM format
    end: str | None = Field(None, pattern=r"^([01]?[0-9]|2[0-3]):[0-5][0-9]$")    # HH:MM format
...
class CapacityBucket(BaseModel):
    ...
    reserved_label: str | None = None  # e.g., "sibling", "municipality"
...
class CenterProperty(BaseModel):
    ...
    complex_value_type_id: UUID | None = None
...
class Center(BaseModel):
    ...
    def get_property(self, property_key: str) -> CenterProperty | None:
...
class ParentPreference(BaseModel):
    ...
    def is_absolute(self) -> bool:
        ...
        return self.strength in [PreferenceStrength.REQUIRED, PreferenceStrength.EXCLUDE]
    ...
    def computed_weight(self) -> float:
        ...
        strength_weights = {
            ...
            PreferenceStrength.EXCLUDE: -1.0
        }
    ...
    def constraint_type(self) -> ConstraintType:
        ...
        elif self.strength == PreferenceStrength.EXCLUDE:
